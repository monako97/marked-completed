...
module.exports = class Tokenizer {
  ...
  mark(src) {
    const cap = this.rules.inline.mark.exec(src);
    if (cap) {
      if (cap[0].length > 1) {
        return {
          type: 'mark',
  ...
  sup(src) {
    const cap = this.rules.inline.sup.exec(src);
    if (cap) {
      if (cap[0].length > 1) {
        return {
          type: 'sup',
  ...
  sub(src) {
    const cap = this.rules.inline.sub.exec(src);
    if (cap) {
      if (cap[0].length > 1) {
        return {
          type: 'sub',
  ...
  s(src) {
    const cap = this.rules.inline.s.exec(src);
    if (cap) {
      if (cap[0].length > 1) {
        return {
          type: 's',
